...
\title[LATTE]{LATTE: A Structure-Informed Latent Model for Protein Sequence Embedding}
...
\abstract{\textbf{Background:}  Deep protein models benefit from embeddings that capture sequence structure relationships. We introduce LATTE, a structure-aware Latent autoencoder trained with a novel structural loss to explicitly learn inter-residue similarity. 
%
\textbf{Results:} LATTE reconstructs unseen UniRef50 sequences with 97.17\% accuracy, remains robust to simulated noise, and generates novel proteins sharing only 10\% identity with training data. Its latent space is informative for downstream tasks: Gaussian process models trained on the latent space achieve fivefold CV accuracy of 0.987 for fluorescent-protein (FP) vs non-FP classification and predict excitation/emission peaks with RMSEs of 2.7 nm and 3.8 nm. Compared with ESM-2, pairwise cosine-distance distributions on matched subsets are broader and heavier-tailed for LATTE (mean = 0.1694, SD = 0.3428; p50 = 0.0141, p90 = 0.9006; n = 49{,}847) versus tighter for ESM-2 (mean = 0.0381, SD = 0.0822; p50 = 0.00953, p90 = 0.1133; n = 49{,}847). Direct pairwise comparisons yield a regression slope $b = 0.125$ and intercept $a = 0.017$ with high rank correlation (Spearman $\rho$=0.761), indicating that LATTE largely preserves neighbor ordering while expanding dynamic range. In $k = 3$ clustering, cosine-based silhouette scores favor LATTE, and cross-partition agreement indicates strong consistency between the two representations.
%
 \textbf{Conclusion:} LATTE provides a structure-aware embedding with preserved neighbor ordering and expanded dynamic range, improving latent-space prefiltering prior to alignment (“Deep BLAST”), while supporting accurate reconstruction and effective downstream prediction.

}
\keywords{protein language model; variational autoencoder; structural loss; fluorescent proteins; embedding geometry; cosine distance; generative modeling; bioinformatics}
...
LATTE is intended as a lightweight, structure-informed generator that complements such foundation models by emphasizing controllable latent structure and efficient training.
...
\subsection{Segment Tree (Generic Prefilter)}
A segment tree hierarchically partitions a 1D domain to answer range queries (e.g., sum, min/max) and support point updates in $O(\log n)$ time~\cite{deBerg2008,Wang2018}. 
As a generic \emph{prefilter}, it can prune large candidate sets using bounds or aggregated statistics before any high-cost procedure (e.g., precise alignment) is applied. 
In this work we rely only on standard properties and complexities of the segment tree; implementation variants are out of scope.
...
LATTE’s latent space remains active and enables true de novo sequence generation, achieving a mean KL~divergence per dimension of 0.04998 (RMSE 0.07027). We confirmed absence of posterior collapse by sampling random latent vectors values around 0.05 to ensure decoder utilization and comparing decoded sequences against originals, demonstrating that LATTE produces genuinely novel protein sequences rather than memorizing the training data.
...
supporting that LATTE preserves ESM’s neighbor ordering while expanding the dynamic range, a property we exploit for latent prefiltering in Deep BLAST.
...
\begin{minipage}{0.48\linewidth}
  \centering
  \includegraphics[width=\linewidth]{img/figure3_hist.png}
\end{minipage}\hfill
\begin{minipage}{0.48\linewidth}
  \centering
  \includegraphics[width=\linewidth]{img/figure3_ecdf.png}
\end{minipage}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\linewidth]{img/figure4.png}
...
\begin{minipage}{0.48\linewidth}
  \centering
  \includegraphics[width=\linewidth]{img/figure5_LATTE.png}
\end{minipage}\hfill
\begin{minipage}{0.48\linewidth}
  \centering
  \includegraphics[width=\linewidth]{img/figure5_ESM.png}
\end{minipage}
...
\begin{figure*}[!ht]
 \centering
 \subfloat[CE over epochs]{\includegraphics[width=0.45\textwidth]{img/figure6_CE.png}}
\subfloat[KL over epochs]{\includegraphics[width=0.45\textwidth]{img/figure6_KL.png}}
